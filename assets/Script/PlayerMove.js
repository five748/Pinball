...
cc.Class({
    ...
    startTouch(event) {
        if (this.gameManage.startGame) {
            ...
            this._touchBool = true;
            
        }
        console.log(this.touchNum+"-------f--------")
        if (this._ratationBool) {
           

            this.OffMove_Anim.stop()
            this.setRotationPower(this.bangBall.position.x)
            if (this.gameManage.startGame) {
    ...
    setRotationPower(index) {
        this._ratationC = index / 95
    },

    moveTouch(event) {
        if (this._touchBool && this._ratationBool == false) {
    ...
    shootPlayer(self) {
        ...
        this.rigidbody.angularVelocity = 2000 *this._ratationC
        this._backGround.pauseSystemEvents()
        this._ratationBool = false
    },
});
